...
var Options struct {
	...
	IronicIgnitionConfig           ignition.IronicIgniotionConfig
}
...
func main() {
	...
	go func() {
		if Options.EnableKubeAPI {
			bmoUtils := controllers.NewBMOUtils(ctrlMgr.GetClient(),
				log.WithField("pkg", "baremetal_operator_utils"),
				Options.EnableKubeAPI)
			useConvergedFlow := bmoUtils.ConvergedFlowAvailable(Options.MinimalVersionForConvergedFlow)
			...
			if useConvergedFlow {
				...
				failOnError((&controllers.PreprovisioningImageReconciler{
					...
					Installer:              bm,
					CRDEventsHandler:       crdEventsHandler,
					IronicIgniotionBuilder: ignition.NewIronicIgniotionBuilder(Options.IronicIgnitionConfig),
					Format:                 Options.InfraEnvConfig.ImageType,
					IronicServiceURL:       ironicBaseURL,
				}).SetupWithManager(ctrlMgr), "failed to create ceontroller PreprovisioningImage")
			}
