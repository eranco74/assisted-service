...
type API interface {
	...
	ListBundles() []models.BundleName
	// GetBundle returns the bundle object for  a specific bundleName
...
func (mgr *Manager) ListBundles() []models.BundleName {
	return []models.BundleName{
		models.BundleNameVirtualization,
		models.BundleNameOpenshiftai,
	}
}

// GetBundle returns the operators associated with a specific bundle
func (mgr *Manager) GetBundle(bundleName string) (*models.Bundle, error) {
	...
	bundle := models.Bundle{Name: models.BundleName(bundleName)}
	for _, operator := range mgr.olmOperators {
