...
var _ = Describe("Operators manager", func() {
	...
	Context("Bundles", func() {
		...
		It("GetOperatorsByBundle should return the operators associated with a specific bundle", func() {
			...
			Expect(bundle.Operators).To(HaveLen(0))
			bundle, err = manager.GetBundle(string(models.BundleNameVirtualization))
			Expect(err).ToNot(HaveOccurred())
			Expect(bundle.Operators).To(HaveLen(2))
			Expect(bundle.Operators).To(ContainElements(cnvOperator.GetName(), odfOperator.GetName()))

			bundle, err = manager.GetBundle(string(models.BundleNameOpenshiftai))
			Expect(err).ToNot(HaveOccurred())
