...
func (e *Events) handleEventSubscription(clusterID *strfmt.UUID, event common.Event) {
	eventSubscriptionList := []models.EventSubscription{}
	...
	for _, es := range eventSubscriptionList {
		e.callEventSubscriptionURL(es, event)
	}
...
func (e *Events) callEventSubscriptionURL(eventSubscription models.EventSubscription, event common.Event) {
	...
	if err != nil {
		...
		if err != nil {
			e.log.WithError(err).Errorf("Failed decoding event subscription callback respose")
			eventSubscription.Status = swag.String(err.Error())
		}
		e.log.Infof("event subscription callback success: %s", res["json"])
	}
}
