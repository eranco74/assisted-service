...
func (r *PreprovisioningImageReconciler) getIronicIgnitionConfig(log logrus.FieldLogger, infraEnv aiv1beta1.InfraEnv) (string, error) {
	config, err := r.IronicIgniotionBuilder.GenerateIronicConfig(r.IronicServiceURL, infraEnv, "")
	if err != nil {
...
func (r *PreprovisioningImageReconciler) SetupWithManager(mgr ctrl.Manager) error {
	mapInfraEnvPPI := func(a client.Object) []reconcile.Request {
		...
		shouldReconcileImage := func(image *metal3_v1alpha1.PreprovisioningImage, infraEnv *aiv1beta1.InfraEnv) bool {
			if infraEnv.Status.ISODownloadURL == "" {
				log.Info("InfraEnv corresponding to the PreprovisioningImage has no image URL available.")
				return false
			}
...
func (r *PreprovisioningImageReconciler) AddIronicAgentToInfraEnv(ctx context.Context, log logrus.FieldLogger, infraEnv *aiv1beta1.InfraEnv) (ctrl.Result, error) {
	...
	conf, err := r.getIronicIgnitionConfig(log, *infraEnv)
	if err != nil {
		return ctrl.Result{}, err
	}

	_, err = r.Installer.UpdateInfraEnvInternal(ctx, installer.UpdateInfraEnvParams{InfraEnvID: *infraEnvInternal.ID}, &conf)
	if err != nil {
		return ctrl.Result{}, err
	}
